...
void decomp_2d_init(int nx, int ny, int nz, int p_row, int p_col, std::array<bool, 3> periodic_bc)
{
...
void decomp_2d_abort(int errorcode, const char *msg)
{
    int nrank;
...
void decomp_info_init(decomp_2d_info *decomp, int nx, int ny, int nz)
{
    ...
    partition(n, pdim, coords, 
              ...
              decomp->xst.data(), decomp->xen.data(), decomp->xsz.data());
    pdim[0] = 1; pdim[1] = 0; pdim[2] = 2;
    partition(n, pdim, coords, 
              decomp->x1st, decomp->x1en, decomp->x1dist,
              decomp->z2st, decomp->z2en, decomp->z2dist,
              decomp->yst.data(), decomp->yen.data(), decomp->ysz.data());
    pdim[0] = 1; pdim[1] = 2; pdim[2] = 0;
    partition(n, pdim, coords, 
              decomp->x1st, decomp->x1en, decomp->x1dist,
              decomp->y2st, decomp->y2en, decomp->y2dist,
              decomp->zst.data(), decomp->zen.data(), decomp->zsz.data());
    ...
    if (buf_size > decomp_buf_size) {
        ...
        if (work1_r != NULL) sycl::free(work1_r, mod_oneapi::queue);
        if (work2_r != NULL) sycl::free(work2_r, mod_oneapi::queue);
        work1_r = (DECOMP_2D_REAL *)sycl::aligned_alloc_shared(
            MEM_ALIGN_SIZE, buf_size * sizeof(DECOMP_2D_REAL) + MEM_ALIGN_SIZE,
            mod_oneapi::queue);
        work2_r = (DECOMP_2D_REAL *)sycl::aligned_alloc_shared(
            MEM_ALIGN_SIZE, buf_size * sizeof(DECOMP_2D_REAL) + MEM_ALIGN_SIZE,
            mod_oneapi::queue);
...
static void mem_split_xy_real(DECOMP_2D_REAL *in, int n1, int n2, int n3, DECOMP_2D_REAL *out, int iproc, int *x1st, int *x1en, int *x1disp) {
    ...
    for (int m = 0; m < iproc; m++) {
      ...
      int pos = x1disp[m];
      int s1 = i2 - i1;
      auto r = sycl::range(n3, n2, s1);
      ev_list.emplace_back(
          mod_oneapi::queue.parallel_for(r, [=](sycl::item<3> idx) {
            auto [i, j, k] = mod_oneapi::unpack<0>(idx);
            int ii = i + i1;
            out[pos + idx.get_linear_id()] =
                in[ii + j * in_offset2 + k * in_offset1];
          }));
    }
    sycl::event::wait(ev_list);
}
...
static void mem_split_yx_real(DECOMP_2D_REAL *in, int n1, int n2, int n3, DECOMP_2D_REAL *out, int iproc, int *y1st, int *y1en, int *y1disp) {
    ...
    for (int m = 0; m < iproc; m++) {
      ...
      int pos = y1disp[m];
      int s2 = i2 - i1;
      auto r = sycl::range(n3, s2, n1);
      ev_list.emplace_back(
          mod_oneapi::queue.parallel_for(r, [=](sycl::item<3> idx) {
            auto [i, j, k] = mod_oneapi::unpack<0>(idx);
            int jj = j + i1;
            out[pos + idx.get_linear_id()] =
                in[i + jj * in_offset2 + k * in_offset1];
          }));
    }
    sycl::event::wait(ev_list);
}
    ...
    for (int m = 0; m < iproc; m++) {
      ...
      ev_list.emplace_back(
          mod_oneapi::queue.parallel_for(r, [=](sycl::item<3> idx) {
            auto [i, j, k] = mod_oneapi::unpack<0>(idx);
            int jj = j + i1;
            out[i + jj * out_offset2 + k * out_offset1] =
                in[pos + idx.get_linear_id()];
          }));
    }
    sycl::event::wait(ev_list);
}
    ...
    for (int m = 0; m < iproc; m++) {
      ...
      ev_list.emplace_back(
          mod_oneapi::queue.parallel_for(r, [=](sycl::item<3> idx) {
            auto [i, j, k] = mod_oneapi::unpack<0>(idx);
            int ii = i + i1;
            out[ii + j * out_offset2 + k * out_offset1] =
                in[pos + idx.get_linear_id()];
          }));
    }
    sycl::event::wait(ev_list);
    ...
    for (int m = 0; m < iproc; m++) {
      ...
      ev_list.emplace_back(
          mod_oneapi::queue.parallel_for(r, [=](sycl::item<3> idx) {
            auto [i, j, k] = mod_oneapi::unpack<0>(idx);
            int jj = j + i1;
            out[i + jj * out_offset2 + k * out_offset1] = in[pos + idx.get_linear_id()];
          }));
    }
    sycl::event::wait(ev_list);
}
