...
typedef struct DECOMP_2D_INFO {
    ...
    std::array<int, 3> xst, xen, xsz; // x-pencil
    std::array<int, 3> yst, yen, ysz; // y-pencil
    std::array<int, 3> zst, zen, zsz; // z-pencil
...
extern void decomp_2d_init(int nx, int ny, int nz, int p_row, int p_col, std::array<bool, 3> periodic_bc);
extern void decomp_2d_finalize();
extern void decomp_2d_abort(int errorcode, const char *msg);
